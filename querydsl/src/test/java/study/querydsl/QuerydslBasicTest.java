...
//
    ...
    public void fetchJoinNo() {
        ...
        boolean loaded = emf.getPersistenceUnitUtil().isLoaded(findMember.getTeam());// 팀이 로딩된 엔티티인지 여부
    ...
    public void fetchJoinUse() {
        ...
        Member findMember = queryFactory
                .selectFrom(member)
                .join(member.team, team).fetchJoin() // join(), leftJoin() 등 조인 기능 뒤에 fetchJoin() 추가하면 됨
                .where(member.username.eq("member1"))
                .fetchOne();
    ...
    @Test
    public void orderByComplexCase() {
    ...
    // getter , setter 없이 바로 필드로 값이 들어감
    @Test
...
//                        Expressions.stringTemplate("function('lower', {0})", member.username)))
                .where(member.username.eq(member.username.lower())) //ansi 표준에 있는 기능은 왠만하면 있음
                .fetch();
